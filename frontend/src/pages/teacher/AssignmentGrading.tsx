...
const AssignmentGrading = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      const res = await fetch(`http://localhost:5001/api/assignments/${id}/submissions`, {
  ...
  const submitGrade = async () => {
    try {
      const res = await fetch(`http://localhost:5001/api/assignments/${id}/grade/${selectedSub._id}`, {
        method: 'POST',
        headers: { 
              ...
              <div className="mb-6">
                ...
                <div className="space-y-2">
                  {selectedSub.files?.map((f: any, i: number) => (
                    <a key={i} href={`http://localhost:5001${f.url}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-emerald-600 hover:underline text-sm font-medium p-3 bg-emerald-50 rounded-xl">
