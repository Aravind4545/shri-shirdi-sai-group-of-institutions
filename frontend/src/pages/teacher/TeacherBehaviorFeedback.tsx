...
const TeacherBehaviorFeedback = () => {
  ...
  const fetchActiveRequestAndStudents = async () => {
    try {
      const reqRes = await fetch('/api/behavior/requests/active', {
      ...
      if (reqRes.ok) {
        ...
        if (reqData) {
          const stdRes = await fetch('/api/behavior/students', {
            headers: { 'x-auth-token': localStorage.getItem('token') || '' }
          });
          if (stdRes.ok) {
            ...
            const fbRes = await fetch('/api/behavior/feedbacks/me/${reqData._id}', {
              headers: { 'x-auth-token': localStorage.getItem('token') || '' }
            });
            if (fbRes.ok) {
  ...
  const submitFeedback = async (studentId: string) => {
    ...
    try {
      const res = await fetch('/api/behavior/feedbacks', {
        method: 'POST',
        headers: {
